...
# X_0 = X[y['DAMAGE_LOCATION']==0]
# y_0 = y[y['DAMAGE_LOCATION']==0]
...
# X2_train_scaled = np.array(X2_train)
# X2_test_scaled = np.array(X2_test)
...
def convert_baseline_model(file_name):
    ...
    key_mapping = {
        ...
        "transformer": "pretrain_transformer",
        "fc1": "pretrain_fc1",
        "fc2": "pretrain_fc2"
    }
    new_param_dict = dict()
...
class DomainTransformer(nn.Module):
    def __init__(self, input_dim, hidden_num, num_classes1, num_classes2):
        ...
        self.pred_loc = nn.Sequential(nn.Linear(hidden_num, 19),
        )
        self.pred_ori = nn.Sequential(nn.Linear(hidden_num, 4),
        )
        ...
        self.pretrain_embedding = nn.Linear(input_dim, 128)
        self.pretrain_transformer = nn.TransformerEncoder(nn.TransformerEncoderLayer(d_model=128, nhead=4), num_layers=2)
        self.pretrain_fc1 = nn.Linear(128, num_classes1)
        self.pretrain_fc2 = nn.Linear(128, num_classes2)
        # for p in self.parameters():
        #     p.requires_grad=False
        for p in self.pretrain_embedding.parameters():
            p.requires_grad=False
    ...
    def forward(self, x):
        ...
        # print('x.shape=========', x.shape)
        x_fc1 = self.domain_trans_in(x)
        # x_fc1_loc = self.pred_loc(x_fc1)
        # x_fc1_ori = self.pred_ori(x_fc1)
        x_fc2 = self.domain_trans_out(x_fc1)
        # print('x.shape=========', x.shape)
        #合并
        x = torch.cat((x_cond, x_fc2), 1)
        # print('x.shape=========', x.shape)
        x = self.pretrain_embedding(x)
        x = x.unsqueeze(1)
        x = self.pretrain_transformer(x)
        x = x.squeeze(1)
        x1 = self.pretrain_fc1(x)
        x2 = self.pretrain_fc2(x)
        return x1, x2
...
# model.load_state_dict(torch.load('domaintransmodel_transformer_v1_in6.pth'))
...
# optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=learning_rate, nesterov=True)
...
def closure():
    ...
    for data, target in train_loader:
        ...
        # loss_mid1 = criterion(pred_loc, target[:,0])
        # loss_mid2 = criterion(pred_ori, target[:,1])
...
torch.save(model.state_dict(), 'domaintrans_model_v1_7524-6011.pth')#只保存模型权重参数，不保存模型结构
